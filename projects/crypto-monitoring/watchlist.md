# Watchlist
_Active holdings and tickers. Updated as positions change._
_Use /market-check [ticker] for live price + news + sentiment._

Last Updated: 2026-03-10

---

## 🟠 Kraken Holdings (USD)

### BTC — Bitcoin
- **Platform:** Kraken
- **Current Interest:** Hold (monitoring)
- **Balance:** 0.00009795 BTC
- **Avg Buy Price:** $90,111.41 ← bought near peak
- **Current Price:** ~$68,402
- **Unrealized P&L:** -$2.13
- **Notes:** Deep in red on avg buy. Long-term hold unless significant recovery signal.

### ETH — Ethereum
- **Platform:** Kraken
- **Current Interest:** Hold (monitoring)
- **Balance:** 0.00187 ETH
- **Avg Buy Price:** $3,151.62
- **Current Price:** ~$2,010
- **Unrealized P&L:** -$2.13
- **Notes:** Also bought above current price. Hold and monitor.

### SOL — Solana
- **Platform:** Kraken
- **Current Interest:** Watch
- **Balance:** <0.00001 SOL (tiny)
- **Avg Buy Price:** $151.10
- **Current Price:** ~$84.67
- **Notes:** Tiny position. Also on Revolut with larger exposure.

### PENGU — Pudgy Penguins
- **Platform:** Kraken
- **Current Interest:** Hold/Watch
- **Balance:** 1,873.13 PENGU
- **Avg Buy Price:** ~$0.007
- **Current Price:** $0.0071 (2026-03-10)
- **Position Value:** ~$13.30
- **Notes:** Meme/NFT token. ATH ~$0.07 (Dec 2024). Currently -90% from ATH.
- **Alert DOWN:** $0.003 | **Alert UP:** $0.015 / $0.04

### EIGEN — EigenLayer
- **Platform:** Kraken
- **Current Interest:** Watch
- **Balance:** 0.002861 EIGEN
- **Avg Buy Price:** $0.3018
- **Current Price:** $0.1858
- **Notes:** Restaking protocol. Down from avg. Small position.

### BABY — Babylon
- **Platform:** Kraken
- **Current Interest:** Watch
- **Balance:** 0.00308 BABY
- **Avg Buy Price:** $0.02815
- **Current Price:** $0.0162 (2026-03-10) +13.64% today
- **Unrealized P&L:** -$0.000374 (tiny position)
- **Notes:** BTC staking protocol. Down from avg. Bounced today.
- **Alert DOWN:** $0.008 | **Alert UP:** $0.025 (avg buy) / $0.05

---

## 🟣 Revolut Holdings (EUR)

### BTC — Bitcoin
- **Platform:** Revolut
- **Current Interest:** Hold
- **Balance:** 0.00014503 BTC
- **Current Price:** ~€59,007
- **Position Value:** ~€8.55
- **24h:** -2.14%

### BCH — Bitcoin Cash
- **Platform:** Revolut
- **Current Interest:** Hold/Watch
- **Balance:** 0.01896662 BCH
- **Current Price:** $447.72 / ~€415 (2026-03-10)
- **Position Value:** ~$8.49
- **Notes:** Secondary Bitcoin fork. Monitor for exit opportunity.
- **Alert DOWN:** $250 | **Alert UP:** $450 / $600

### SOL — Solana
- **Platform:** Revolut
- **Current Interest:** Hold
- **Balance:** 0.042863 SOL
- **Current Price:** ~€73
- **Position Value:** ~€3.13
- **24h:** -3.82%

### Revolut Brokerage
- **Platform:** Revolut Invest
- **Current Value:** €14.19 (+1.68% today)
- **Notes:** Brokerage account — check holdings in Revolut app directly.

---

## 🔭 On Radar (Not Holding)
_Tickers to watch but not currently in portfolio_

| Ticker | Reason to Watch |
|--------|----------------|
| XRP | Consistently high volume, regulatory clarity improving |
| DOGS | Showed up in Revolut top movers (+28%) |
| FLOW | Top mover on Revolut (+18.9%) |

---

## 🆕 OPN — Opinion Labs
_Added: 2026-03-09 | Status: WATCHING — small entry, monitoring sell pressure_

- **Platform:** Revolut (BEP-20 / BNB Chain — listed on Binance March 5, 2026)
- **Entry Date:** 2026-03-09 | **Avg Buy Price:** $0.3326
- **Balance:** 3.498 OPN | **Position Value:** ~$1.10
- **Unrealized P&L:** -$0.0671 (-5.77%)
- **Current Price:** $0.338 (2026-03-10) +10.67% today — first day above avg buy
- **ATH:** $0.75 (Feb 23, 2026) — currently **-58% from ATH**
- **Market Cap:** $62.2M | **24H Volume:** $137.1M | **Market Rank:** #387
- **FDV:** ~$320M | Circulating: 200M / 1B total (20% only)

### Alert Thresholds (run `python projects/crypto-monitoring/price-alert.py`)
| Direction | Price | Action |
|-----------|-------|--------|
| DOWN | **$0.18** | Potential accumulation zone — airdrop pressure may exhaust |
| UP | **$0.45** | Recovery signal — re-evaluate and consider adding |
| UP | **$0.60** | Near ATH resistance — consider reducing / taking profit |

### Key Dates to Watch
- **~Oct 2026** — 7-month airdrop vest completes, sell pressure should ease
- **Q3 2026** — check for real platform usage (prediction market volume)
- **March 2027** — team + investor cliff: 425M tokens unlock (2x current supply)

### Thesis
Solid Tier-1 backing (Binance Labs, Jump Crypto, Hack VC). Real product. Currently in post-launch
distribution phase. Not a scam but wrong timing for large entry. Better window: $0.15–$0.20.
Full research: `projects/crypto-monitoring/research/2026-03-09-OPN-opinion-labs.md`

---

## Archived (Exited Positions)
_Move entries here when sold_
